...
mount_hard_disk() {
    ...
    if is_hard_disk_available; then
        ...
        if [ $? -ne 0 ]; then
            notify-send -u critical Backup "Faild to mount $HardDisk Hard Disk to the system"
            exit 1
        fi

        notify-send -u critical Backup "Mounted $HardDisk successfully."

        main
    
    else

        notify-send -u critical Backup "The $HardDisk Hard Disk is not connected to the system"
        exit 1
...
delete_older_backups() {
    ...
    if [ $? -eq 0 ]; then
        ...
        notify-send -u critical Backup "Failed to delete older backup folders."
    fi
}
...
main() {
    ...
    if is_mounted; then
        ...
        cd "$Mount_Point/$Browse/" || {
            notify-send -u critical Backup "Failed to access directory: $Mount_Point/$Browse/"
            exit 1
        }

        mkdir -p "$Folder" || {
            notify-send -u critical Backup "Failed to create directory: $Folder"
            exit 1
        }

        sshpass -p "$Password" scp -r "$Address:$Folder1" "$Folder/" || {
            notify-send -u critical Backup "Failed to copy files from server."
            exit 1
        }

        notify-send -u critical Backup "Files copied successfully to $Folder."
